...
export class PaintManager {
    ...
    // Callback xử lý khi tương tác sai (chạm vào vật thể isCorrect = false)
    private onWrongInteract?: () => void;

    constructor(
        scene: Phaser.Scene, 
        onComplete: (id: string, rt: Phaser.GameObjects.RenderTexture, usedColors: Set<number>) => void,
        onWrongInteract?: () => void
    ) {
        this.scene = scene;
        this.onPartComplete = onComplete;
        this.onWrongInteract = onWrongInteract;
    ...
    public createPaintableLayer(x: number, y: number, key: string, scale: number, uniqueId: string): Phaser.GameObjects.Image {
        ...
        hitArea.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
            // 🔥 CHECK IS CORRECT 🔥
            // Nếu vật thể được đánh dấu là SAI (isCorrect = false), thì không cho tô và báo lỗi
            const isCorrect = hitArea.getData('isCorrect');
            if (isCorrect === false) {
                 if (this.onWrongInteract) this.onWrongInteract();
                 return;
            }
