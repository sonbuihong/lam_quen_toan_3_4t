...
export class ObjectManager {
    ...
    public spawnObjectsFromConfig(configData: any) {
        ...
        configData.images.forEach((def: any) => {
             ...
             img.setData('id', def.id || key);
             
             if (def.isDecorative) {
                 img.setData('isDecorative', true);
                 img.setData('isTarget', false);
             } else {
                 img.setData('isTarget', true);
             }
             
             if (def.isHiddenHitbox) {
                 img.setAlpha(0);
                 img.setData('isHiddenHitbox', true);
             }

             if (def.baseScale) img.setScale(def.baseScale);
             if (def.baseScaleX !== undefined) img.scaleX = def.baseScaleX;
             if (def.baseScaleY !== undefined) img.scaleY = def.baseScaleY;
             if (def.angle !== undefined) img.setAngle(def.angle);
    ...
    public isWrongAnswer(object: Phaser.GameObjects.Image): boolean {
        const id = object.getData('id') || object.getData('textureKey');
        return !this.correctKeys.includes(id);
    }
