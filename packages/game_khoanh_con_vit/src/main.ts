...
import { installIrukaE2E } from './e2e/installIrukaE2E';
    ...
    function attachResetHandler() {
        const resetBtn = document.getElementById('btn-reset') as HTMLImageElement;
        let isResetting = false; // Debounce flag - chống nhấn reset liên tục gây race condition
        if (resetBtn) {
            resetBtn.onclick = () => {
                // Guard: không cho phép nhấn reset khi đang xử lý reset trước đó
                ...
                gamePhaser.scene.stop(SceneKeys.Scene2);
                gamePhaser.scene.stop(SceneKeys.UI);
                gamePhaser.scene.stop(SceneKeys.EndGame);
                
                gamePhaser.scene.stop(SceneKeys.Scene1);
                gamePhaser.scene.start(SceneKeys.Scene1, { isRestart: true }); 
                
                // Cho phép nhấn reset lại sau 1 giây (debounce)
