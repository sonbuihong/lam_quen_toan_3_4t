...
export default class Scene1 extends Phaser.Scene {
    ...
    create() {
        ...
        // Đếm tổng số cặp cần nối (= số pairs trong config)
        this.totalTargets = levelConfig.pairs ? levelConfig.pairs.length : this.objectManager.getImageObjects().length;
        console.log(`[Scene1] Màn ${this.currentLevelIndex + 1} - Tổng cặp cần nối: ${this.totalTargets}`);
    ...
    private setupGameplay() {
        // Helper: gắn interactive + pointerdown cho 1 object (dùng chung cho images lẫn answers)
        const makeInteractive = (obj: Phaser.GameObjects.Image) => {
            const radius = Math.max(obj.displayWidth, obj.displayHeight) * 0.5 + 20;
            const hitArea = new Phaser.Geom.Circle(obj.width / 2, obj.height / 2, radius);
            obj.setInteractive({
                hitArea,
            ...
            obj.on('pointerdown', () => {
                if (obj.getData('isMatched')) return;
                this.startDrag(obj);
            });
        };

        // Gắn interactive cho CẢ images lẫn answers (nối 2 chiều)
        this.objectManager.getImageObjects().forEach(makeInteractive);
        this.objectManager.getAnswerObjects().forEach(makeInteractive);
    ...
    /**
     * Tìm tất cả các đích hợp lệ mà nguồn có thể nối vào (hỗ trợ 2 chiều).
     * - Nguồn là image -> trả về answers khớp cặp.
     * - Nguồn là answer -> trả về images khớp cặp.
     */
    private getValidTargetsForSource(sourceImage: Phaser.GameObjects.Image): Phaser.GameObjects.Image[] {
        const fullConfig = this.cache.json.get(DataKeys.LevelS1Config);
        const rawLevelConfig = fullConfig.levels ? fullConfig.levels[this.currentLevelIndex] : fullConfig;

        const sourceKey = sourceImage.texture.key;
        const isSourceImage = this.objectManager.getImageObjects().includes(sourceImage);
        const possibleTargets: Phaser.GameObjects.Image[] = [];

        if (rawLevelConfig.matchMode === 'any' && rawLevelConfig.answers) {
            // matchMode 'any': mọi image đều nối được mọi answer
            const targets = isSourceImage
                ? this.objectManager.getAnswerObjects()
                : this.objectManager.getImageObjects();
            possibleTargets.push(...targets);
        } else if (rawLevelConfig.pairs) {
            // Tra cứu cặp theo CẢ 2 chiều
            for (const p of rawLevelConfig.pairs) {
                if (isSourceImage && p.image === sourceKey) {
                    const target = this.objectManager.getAnswerObjects().find((o: Phaser.GameObjects.Image) => o.texture.key === p.answer);
                    if (target) possibleTargets.push(target);
                } else if (!isSourceImage && p.answer === sourceKey) {
                    const target = this.objectManager.getImageObjects().find((o: Phaser.GameObjects.Image) => o.texture.key === p.image);
                    if (target) possibleTargets.push(target);
                }
            }
        } else {
            ...
            if (isSourceImage) {
                const ans = this.objectManager.getAnswerObjects().find((o: Phaser.GameObjects.Image) => o.texture.key === expectedAnswerKey);
                if (ans) possibleTargets.push(ans);
            }
        }
    ...
    private getHintTargetPair(): { source: Phaser.GameObjects.Image, target: Phaser.GameObjects.Image } | null {
        ...
        const validTargets = this.getValidTargetsForSource(source);
        const target = validTargets.find(t => !t.getData('isMatched'));
    ...
    private evaluateDrop(dropX: number, dropY: number) {
        ...
        // Xác định nhóm đối diện: kéo từ image -> tìm answer, kéo từ answer -> tìm image
        const isSourceImage = this.objectManager.getImageObjects().includes(this.currentDragSource);
        const oppositeGroup = isSourceImage
            ? this.objectManager.getAnswerObjects()
            : this.objectManager.getImageObjects();

        // Kiểm tra drop có trúng object đối diện nào không (mở rộng 20px cho dễ nhắm)
        const hitTarget = oppositeGroup.find(t => {
        ...
        if (hitTarget.getData('isMatched')) {
            this.handleWrongDrop(null);
            return;
        }

        // Kiểm tra cặp có khớp config không (hỗ trợ cả 2 chiều)
        const validTargets = this.getValidTargetsForSource(this.currentDragSource);
        const isValidHit = validTargets.includes(hitTarget);

        if (isValidHit) {
            this.handleCorrectDrop(hitTarget);
        } else {
            this.handleWrongDrop(hitTarget);
        }
    }
    ...
    private handleCorrectDrop(targetImage: Phaser.GameObjects.Image) {
        ...
        const targetKey = targetImage.texture.key;
        const edgeStart = this.connectionManager.getContentEdgePoint(this.currentDragSource, targetImage.x, targetImage.y);
        const edgeEnd = this.connectionManager.getContentEdgePoint(targetImage, this.currentDragSource.x, this.currentDragSource.y);
        ...
        // Đánh dấu matched + tắt interactive CẢ 2 PHÍA (nối 2 chiều)
        this.currentDragSource.setData('isMatched', true);
        targetImage.setData('isMatched', true);
        this.currentDragSource.disableInteractive();
        targetImage.disableInteractive();

        // Vẽ đường nối CỐ ĐỊNH (ngắn nhất, chạm pixel thật) từ nguồn tới đích
        this.connectionManager.drawFinalLine(this.currentDragSource, targetImage, 0x00ff00);

        // Hiệu ứng phóng to - thu nhỏ cả 2 object
        this.tweens.add({
            targets: [this.currentDragSource, targetImage],
    ...
    private completeLevel() {
        ...
        // Tắt toàn bộ input của các object còn lại (cả images lẫn answers)
        this.objectManager.getImageObjects().forEach(img => img.disableInteractive());
        this.objectManager.getAnswerObjects().forEach(ans => ans.disableInteractive());
     ...
     * @param hitTarget object đối diện bị hit (null nếu thả ra ngoài)
     */
    private handleWrongDrop(hitTarget: Phaser.GameObjects.Image | null) {
        ...
        if (this.currentDragSource) {
            ...
            if (hitTarget) {
                // Thả vào đích sai -> WRONG_PAIR
                const targetKey = hitTarget.texture.key;
                const edgeStart = this.connectionManager.getContentEdgePoint(this.currentDragSource, hitTarget.x, hitTarget.y);
                const edgeEnd = this.connectionManager.getContentEdgePoint(hitTarget, this.currentDragSource.x, this.currentDragSource.y);
                const pathLength = Math.round(Phaser.Math.Distance.Between(edgeStart.x, edgeStart.y, edgeEnd.x, edgeEnd.y));
            ...
            // Hiệu ứng rung lắc object nguồn
            this.tweens.add({
