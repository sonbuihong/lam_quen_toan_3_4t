...
export class ObjectManager {
    ...
    public spawnObjectsFromConfig(configData: any) {
        ...
        if (configData.matchMode === 'any' && configData.images) {
            this.correctKeys = configData.images.map((img: any) => img.textureKey);
        } else if (configData.pairs) {
            this.correctKeys = configData.pairs.map((p: any) => p.image);
        } else if (Array.isArray(configData.correctKey)) {
        ...
        configData.images.forEach((def: any) => {
             ...
             img.setData('hitRadius_pct', def.hitRadius_pct); // Lưu data để Scene1 dùng cấu hình hitbox
        ...
        if (configData.answers) {
            configData.answers.forEach((def: any) => {
                 ...
                 img.setData('hitRadius_pct', def.hitRadius_pct);
                 if (def.baseScale) img.setScale(def.baseScale);
