...
export default class Scene1 extends Phaser.Scene {
    ...
    private handleLassoSelection(polygon: Phaser.Geom.Polygon) {
        ...
        if (isSuccess) {
            // --- TRƯỜNG HỢP ĐÚNG (SUCCESS): khoanh bao trọn TẤT CẢ hình đúng ---
            ...
            // Tính bounding box dựa vào TÂM CÁC HÌNH ĐÚNG thay vì mép hình
            // Để loại bỏ sự khác biệt kích thước (do ảnh mũ to hơn ảnh con vật sinh ra viền quá bự)
            const minCenterX = Math.min(...correctObjects.map(o => o.x));
            const maxCenterX = Math.max(...correctObjects.map(o => o.x));
            const minCenterY = Math.min(...correctObjects.map(o => o.y));
            const maxCenterY = Math.max(...correctObjects.map(o => o.y));

            const ellipseCenterX = (minCenterX + maxCenterX) / 2;
            const ellipseCenterY = (minCenterY + maxCenterY) / 2;
            
            // Bán kính = Nửa khoảng cách giữa 2 tâm xa nhất + Padding cố định (bằng nhau ở mọi màn)
            // Padding dùng chung cho tất cả level để vòng xanh to đều nhau
            const basePadding = 280;
            const ellipseRadiusX = (maxCenterX - minCenterX) / 2 + basePadding;
            const ellipseRadiusY = (maxCenterY - minCenterY) / 2 + basePadding - 50;

            // Vẽ 1 ellipse duy nhất bao quanh cả 2 hình
            const graphics = this.add.graphics();
            graphics.setDepth(100);
            graphics.lineStyle(10, 0x00ff00);
            graphics.strokeEllipse(ellipseCenterX, ellipseCenterY, ellipseRadiusX * 2, ellipseRadiusY * 2);

            AudioManager.stopAll();
            console.log(`✅ Khoanh ĐÚNG! Đã khoanh đủ ${correctObjects.length} hình.`);
            AudioManager.play("sfx-correct");

            this.objectManager.highlightObjects(correctObjects, true);
            this.stopActiveHint();

            // Ghi nhận điểm SDK (tạm thời không finalize ở đây - Scene2 sẽ finalize)
            // Không gọi game.finalizeAttempt() ở đây vì game chưa kết thúc - còn Scene2

            // Vô hiệu hóa input ngay
            this.lassoManager.disable();

            // Dữ liệu ellipse + levelIndex để truyền sang Scene2
            const ellipsePayload = {
            ...
            // Đợi WIN_DELAY rồi chuyển sang Scene2 (game kéo nối)
            const t = GameConstants.SCENE1.TIMING.WIN_DELAY;
            this.time.delayedCall(t, () => {
                // pause(Scene1): Scene1 bị tạm dừng nhưng VẪN RENDER
                // board, hình ảnh, ellipse xanh đều còn nguyên trên màn hình.
                // Scene2 chỉ cần thêm drag line + answers lên trên, không spawn lại gì.
            ...
            // Rung chỉ các hình cần khoanh (không rung ảnh đáp án phía dưới)
            const imageObjs = this.objectManager.getImageObjects();
    ...
    private getEllipseParams() {
        const correctObjects = this.objectManager.getImageObjects().filter(obj => this.objectManager.isCorrectAnswer(obj));
        if (correctObjects.length === 0) return null;

        const minCX = Math.min(...correctObjects.map(o => o.x));
        const maxCX = Math.max(...correctObjects.map(o => o.x));
        const minCY = Math.min(...correctObjects.map(o => o.y));
        const maxCY = Math.max(...correctObjects.map(o => o.y));
        const basePadding = 280;

        return {
            cx: (minCX + maxCX) / 2,
            cy: (minCY + maxCY) / 2,
            rx: (maxCX - minCX) / 2 + basePadding,
            ry: (maxCY - minCY) / 2 + basePadding - 50,
        };
    }
     ...
     */
    private createEllipsePath(cx: number, cy: number, rx: number, ry: number, numPoints = 64): Phaser.Math.Vector2[] {
