...
export class LassoManager {
    ...
    public enable() {
        if (this.isDestroyed) {
            console.warn('[LassoManager] Không thể enable - đã bị destroy.');
            return;
        }
        
        // Đảm bảo không add trùng event (nếu đã enable trước đó)
        this.disable();
    ...
    public disable() {
        ...
        this.clear();
        // Lưu ý: KHÔNG destroy maskGraphics ở đây vì enable() có thể được gọi lại
        // (ví dụ: sau khi trả lời sai, game cho phép vẽ lại).
    }

    /**
     * Giải phóng toàn bộ tài nguyên. Chỉ gọi khi Scene thực sự shutdown.
     */
    public destroyAll() {
